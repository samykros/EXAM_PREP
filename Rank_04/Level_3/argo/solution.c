...
struct pair *parse_map(FILE *stream, int *size) {
    ...
    while ((c = fgetc(stream)) != EOF) {
        ...
        if (c == '}') break; // End of JSON map

        if (c != '"') {
            fprintf(stderr, "Expected key in double quotes\n");
            free(pairs);
            return NULL;
        }

        char *key;
        if (!parse_string(stream, &key)) return NULL;
...
// Main function to implement the argo function
